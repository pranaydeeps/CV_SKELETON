...
#### Some common models are already defined:
#### 1. FCN 8
#### 2. FCN 16
#### 3. FCN 32
#### 4. U-NET
...
class UNetDec(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, dropout=False):
        ...
        layers = [
            nn.Conv2d(in_channels, out_channels, 3, dilation=8, padding = 4),
            nn.BatchNorm2d(out_channels),
            nn.ReLU(inplace=True),
            nn.Conv2d(out_channels, out_channels, 3, dilation=8, padding = 4),
            nn.BatchNorm2d(out_channels),
            nn.ReLU(inplace=True),
        ]
        if dropout:
...
class ReconDec(nn.Module):

    def __init__(self, FINAL_CHANNELS):
        super(ReconDec, self).__init__()
        ...
        self.final = nn.Conv2d(64, FINAL_CHANNELS, 1)
    def forward(self, dec4, dec3, dec2, dec1, center, x):
        ...
        recon_out = F.upsample_bilinear(self.final(enc1), x.size()[2:])
        return recon_out
...
class UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        unet_out = F.upsample_bilinear(self.final(enc1), x.size()[2:])

        recon_out = self.ReconDec.forward(dec4, dec3, dec2, dec1, center, x)

        return unet_out, recon_out


class SegNetEnc(nn.Module):

    def __init__(self, in_channels, out_channels, num_layers):
        super(SegNetEnc).__init__()
